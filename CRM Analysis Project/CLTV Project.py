...
print(cltv_df.groupby("segment").agg({"mean", "sum", "count"}))
...
def change_with_th(dataframe, variable):
    ...
    dataframe[dataframe[variable] > upper_limit][variable] = upper_limit
    dataframe[dataframe[variable] < lower_limit][variable] = lower_limit
...
df["last_order_date"] = pd.to_datetime(-df["Recency"], unit="D", origin=pd.to_datetime("2014-10-29"))
df["Dt_Customer"] = pd.to_datetime(df["Dt_Customer"])

cltv = pd.DataFrame()
cltv["Recency_weekly"] = (df["last_order_date"] - df["Dt_Customer"]).dt.days / 7
cltv["T_weekly"] = (dt.datetime(year=2014, month=10, day=4) - df["Dt_Customer"]).dt.days / 7
cltv["frequency"] = df["Purchase Frequency"]
cltv = cltv[cltv["frequency"] > 1]
cltv["monetary_avg"] = df["total_spent"] / df["Purchase Frequency"]
